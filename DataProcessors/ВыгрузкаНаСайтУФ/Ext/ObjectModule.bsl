Функция СведенияОВнешнейОбработке() Экспорт
    ...
    ПараметрыРегистрации.Наименование = "Выгрузка на сайт";
    ПараметрыРегистрации.Версия = "1.0";
    ПараметрыРегистрации.БезопасныйРежим = Истина;
    ПараметрыРегистрации.Информация = "Выгрузка на сайт УФ";
	ТаблицаКоманд = ПолучитьТаблицуКоманд();
	ДобавитьКоманду(ТаблицаКоманд,
	"Выгрузка на сайт",
	"ВыгрузкаНаСайтУФКоманда",
	"ОткрытиеФормы",
	Истина,"");
...
Функция ЗаполнитьОбъектИзОтветаJSON(Знач ТекстJSON, ТипРезультата = "Соответствие") Экспорт
    Результат = ПолучитьОбъектРезультата(ТипРезультата);
	
	ТекстJSON = СтрЗаменить(ТекстJSON, "\""", """");
	ТекстJSON = СтрЗаменить(ТекстJSON, """", "");	
	Если Лев(ТекстJSON, 1) = "{" Тогда
		ЗаполнитьДанныеИзОтветаJSON(Результат, ТекстJSON, ТипРезультата, ТипРезультата);
	ИначеЕсли Лев(ТекстJSON, 1) = "[" Тогда
		МассивДанных = Новый Массив;
		ЗаполнитьДанныеИзОтветаJSON(МассивДанных, ТекстJSON, "Массив", ТипРезультата);
...
Функция ПолучитьОбъектРезультата(ТипРезультата)
	Если ТипРезультата = "Структура" Тогда
...
Процедура ЗаполнитьДанныеИзОтветаJSON(Результат, ТекстJSON, ТипДанных, ТипРезультата = "Соответствие")
	...
	Пока ТекстJSON <> "" Цикл
		...
		Если ПервыйСимвол = "{" Тогда
			Значение = ПолучитьОбъектРезультата(ТипРезультата);
			ЗаполнитьДанныеИзОтветаJSON(Значение, ТекстJSON, ТипРезультата, ТипРезультата);
			
			Если ТипДанных = ТипРезультата Тогда
		...
		ИначеЕсли ПервыйСимвол = "[" Тогда
			...
			ЗаполнитьДанныеИзОтветаJSON(Значение, ТекстJSON, "Массив", ТипРезультата);
			...
			Иначе
				Результат.Добавить(Значение);
			КонецЕсли;
		ИначеЕсли ПервыйСимвол = "}" И ТипДанных = ТипРезультата Тогда
			ТекстJSON = СокрЛП(Сред(ТекстJSON, 2));
			Если Лев(ТекстJSON, 1) = "," Тогда
				ТекстJSON = СокрЛП(Сред(ТекстJSON, 2));
			КонецЕсли;
